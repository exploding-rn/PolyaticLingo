...
# Force Windows to treat this as a standalone app to display taskbar icons
try:
    myappid = 'polyatic.translator.v14'
...
def get_resource_path(relative_path):
    ...
    return os.path.join(os.path.abspath("."), "polyatic lingo", relative_path)
...
def get_data_file_path():
    ...
    return Path(get_resource_path(DATA_FILE_NAME))
...
def show_previous_translations():
    ...
    text_box = tk.Text(history_window, wrap="word", height=16, width=60, relief="flat", bd=1)
    text_box.pack(fill="both", expand=True, padx=10, pady=10)

    scrollbar = ttk.Scrollbar(history_window, command=text_box.yview)
...
# Custom Clean Arc Theme styling configuration using standard ttk
style = ttk.Style()
style.theme_use("vista")  # Clean base engine
style.configure("TLabel", background="#f5f6f7", foreground="#333333", font=("Segoe UI", 10))
style.configure("TButton", font=("Segoe UI", 10, "bold"), padding=6)
style.configure("TEntry", padding=5)

# Safe window icon load -> FIXED TYPO HERE TOO
try:
    icon_path = get_resource_path("a136f82d.ico")
...
# App Widgets
ttk.Label(root, text="Text to Translate", style="TLabel").pack(pady=8)
entry = ttk.Entry(root, width=40)
entry.pack(pady=5)

ttk.Button(root, text="Translate", command=on_encode).pack(pady=10)

ttk.Label(root, text="Result:", style="TLabel").pack(pady=2)
...
ttk.Button(root, text="View Previous Translations", command=show_previous_translations).pack(pady=5)
